...
{
    public enum PlayerCameraShakeEventType
    {
        Landing,
        GrappleImpact
    }
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        [Header("Rail Exit Momentum Preservation")]
        [SerializeField] private bool preserveRailExitMomentum = true;
        [SerializeField, Range(0f, 2f)] private float railExitMomentumGraceTime = 0.35f;
        ...
        [SerializeField] private float m_grappleLandingShakeSuppressTime = 0.10f;
        ...
        private float m_railExitMomentumGraceTimer;

        public event Action TrickSystemEvent;
        public event Action<PlayerCameraShakeEventType, float> CameraShakeEvent;
        ...
        {
            bool wasGroundedLastFixedFrame = m_bIsGrounded;
            m_bIsGrounded = Physics.CheckSphere(m_tGroundCheck.position, m_fGroundDistance, m_lGround);

            if (m_landingShakeSuppressTimer > 0f)
            {
                m_landingShakeSuppressTimer -= Time.fixedDeltaTime;
            }

            if (m_railExitMomentumGraceTimer > 0f)
            {
                m_railExitMomentumGraceTimer -= Time.fixedDeltaTime;
            }
        ...
        private void UpdateLandingShakeTracking(bool wasGroundedLastFixedFrame)
        {
            if (m_bOnRail)
            {
                m_peakAirborneDownwardSpeed = 0f;
                return;
            }

            if (!m_bIsGrounded)
            {
                float downwardSpeed = Mathf.Max(0f, -rb.linearVelocity.y);
                if (downwardSpeed > m_peakAirborneDownwardSpeed)
                {
                    m_peakAirborneDownwardSpeed = downwardSpeed;
                }

                return;
            }

            if (!wasGroundedLastFixedFrame && m_bIsGrounded)
            {
                if (m_landingShakeSuppressTimer <= 0f && m_peakAirborneDownwardSpeed > 0.01f)
                {
                    CameraShakeEvent?.Invoke(PlayerCameraShakeEventType.Landing, m_peakAirborneDownwardSpeed);
                }

                m_landingShakeSuppressTimer = 0f;
                m_peakAirborneDownwardSpeed = 0f;
                return;
            }
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            {
                float grappleImpactSpeed = collision.relativeVelocity.magnitude > 0f
                    ? collision.relativeVelocity.magnitude
                    : rb.linearVelocity.magnitude;

                CameraShakeEvent?.Invoke(PlayerCameraShakeEventType.GrappleImpact, grappleImpactSpeed);
                m_landingShakeSuppressTimer = m_grappleLandingShakeSuppressTime;
        ...
        private void ExitRailInternal(bool jumpedOff)
        {
            ...
            float preservedExitSpeed = Mathf.Max(exitSpeed, new Vector3(exitVelocity.x, 0f, exitVelocity.z).magnitude);
            ...
            if (preserveRailExitMomentum)
            {
                PreserveRailExitMomentumForNormalController(preservedExitSpeed);
            }
            else
            {
                desiredMoveSpeed = 0f;
                ...
                keepMomentum = false;
                m_railExitMomentumGraceTimer = 0f;
            }
        ...
        private void PreserveRailExitMomentumForNormalController(float preservedExitSpeed)
        {
            float safePreservedSpeed = Mathf.Max(walkSpeed, preservedExitSpeed, railFallbackMinSpeed);

            desiredMoveSpeed = safePreservedSpeed;
            moveSpeed = safePreservedSpeed;
            lastDesiredMoveSpeed = safePreservedSpeed;

            keepMomentum = true;
            speedChangeFactor = 1f;
            m_railExitMomentumGraceTimer = railExitMomentumGraceTime;
        }
