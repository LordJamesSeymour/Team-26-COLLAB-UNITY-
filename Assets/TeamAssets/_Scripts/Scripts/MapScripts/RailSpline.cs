...
namespace Group26.Player.Movement
{
    ...
    public class RailSpline : MonoBehaviour
    {
        ...
        [Header("Side Wall Audio Reactive")]
        [SerializeField] private bool autoAddAudioAnalyzerToSideWalls = true;
        ...
        private void BuildGeneratedGeometry()
        {
            ...
            if (generateSideWallMeshes)
            {
                ...
                if (autoAddAudioAnalyzerToSideWalls)
                {
                    EnsureAudioAnalyzer(leftWallTransform);
                    EnsureAudioAnalyzer(rightWallTransform);
                }
            }
            else
            {
                ...
                RemoveAudioAnalyzer(leftWallTransform);
                RemoveAudioAnalyzer(rightWallTransform);
            }
        ...
        private void EnsureAudioAnalyzer(Transform target)
        {
            if (target == null)
                return;
            ...
            global::AudioAnalyzer analyzer = target.GetComponent<global::AudioAnalyzer>();
            if (analyzer == null)
                analyzer = target.gameObject.AddComponent<global::AudioAnalyzer>();

            analyzer.SetTargetRenderer(renderer);
        }

        private void RemoveAudioAnalyzer(Transform target)
        {
            if (target == null)
                return;
