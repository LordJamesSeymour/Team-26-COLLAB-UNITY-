...
public class settingsmenuscript : menuscreenscript
{
	...
	[Header("Resolution Fix")]
	[SerializeField] private int m_preferredWidth = 1920;
	[SerializeField] private int m_preferredHeight = 1080;
	...
	private Coroutine m_displayModeRoutine;
	...
	private bool m_displayModeChanging;
	private bool m_appliedFullscreen = true;
	...
	protected override void Awake()
	{
		...
		Vector2Int startupResolution = GetPreferredFullscreenResolution();
		ApplyBorderlessFullscreenImmediate(startupResolution);

		m_fullscreenToggle.SetIsOnWithoutNotify(true);
		m_appliedFullscreen = true;

		m_widthInput.interactable = false;
		m_heightInput.interactable = false;

		UpdateResolutionInputFields(startupResolution.x, startupResolution.y);
	...
	private void RefreshResolutionLimits()
	{
		Vector2Int preferredResolution = GetPreferredFullscreenResolution();

		int maxWidth = Mathf.Max(MinWindowWidth, preferredResolution.x, Screen.width, Screen.currentResolution.width);
		int maxHeight = Mathf.Max(MinWindowHeight, preferredResolution.y, Screen.height, Screen.currentResolution.height);

		Resolution[] resolutions = Screen.resolutions;

		for (int i = 0; i < resolutions.Length; i++)
		{
			maxWidth = Mathf.Max(maxWidth, resolutions[i].width);
			maxHeight = Mathf.Max(maxHeight, resolutions[i].height);
		}
	...
	private Vector2Int GetPreferredFullscreenResolution()
	{
		Resolution[] resolutions = Screen.resolutions;

		for (int i = 0; i < resolutions.Length; i++)
		{
			if (resolutions[i].width == m_preferredWidth && resolutions[i].height == m_preferredHeight)
			{
				return new Vector2Int(m_preferredWidth, m_preferredHeight);
			}
		}
		...
		for (int i = 0; i < resolutions.Length; i++)
		{
			...
			if (IsCloseToTargetAspect(width, height))
			{
	...
	private bool IsCloseToTargetAspect(int width, int height)
	{
		if (height <= 0)
			return false;
	...
	private void ApplyBorderlessFullscreenImmediate(Vector2Int resolution)
	{
	...
	private void ApplyWindowedResolutionImmediate(int width, int height)
	{
		RefreshResolutionLimits();
	...
	private void UpdateResolutionInputFields(int width, int height)
	{
		...
		if (m_widthInput != null)
			m_widthInput.SetTextWithoutNotify(m_widthInputText);

		if (m_heightInput != null)
			m_heightInput.SetTextWithoutNotify(m_heightInputText);
	}
	...
	{
		if (m_displayModeChanging)
		{
			m_fullscreenToggle.SetIsOnWithoutNotify(m_appliedFullscreen);
			return;
		}

		if (m_displayModeRoutine != null)
			StopCoroutine(m_displayModeRoutine);

		m_displayModeRoutine = StartCoroutine(ChangeDisplayModeRoutine(m_fullscreenToggle.isOn));
	}

	private IEnumerator ChangeDisplayModeRoutine(bool fullscreen)
	{
		m_displayModeChanging = true;

		if (m_fullscreenToggle != null)
			m_fullscreenToggle.interactable = false;

		RefreshResolutionLimits();

		if (fullscreen)
		{
			Vector2Int targetResolution = GetPreferredFullscreenResolution();

			m_widthInput.interactable = false;
			m_heightInput.interactable = false;

			// This intentionally mimics your manual fix:
			// windowed at correct 16:9 resolution first, then fullscreen.
			Screen.fullScreenMode = FullScreenMode.Windowed;
			...
			UpdateResolutionInputFields(targetResolution.x, targetResolution.y);
		...
		else
		{
			...
			int targetWidth = ReadInputFieldAsResolutionValue(m_widthInput, m_preferredWidth, MinWindowWidth, m_maxWidth);
			int targetHeight = ReadInputFieldAsResolutionValue(m_heightInput, m_preferredHeight, MinWindowHeight, m_maxHeight);

			ApplyWindowedResolutionImmediate(targetWidth, targetHeight);

			m_appliedFullscreen = false;
			m_fullscreenToggle.SetIsOnWithoutNotify(false);
		}

		yield return null;

		if (m_fullscreenToggle != null)
			m_fullscreenToggle.interactable = true;

		m_displayModeChanging = false;
	...
	public void ChangeScreenSize(TMP_InputField input)
	{
		...
		int width = ReadInputFieldAsResolutionValue(m_widthInput, m_preferredWidth, MinWindowWidth, m_maxWidth);
		int height = ReadInputFieldAsResolutionValue(m_heightInput, m_preferredHeight, MinWindowHeight, m_maxHeight);
		...
		ApplyWindowedResolutionImmediate(width, height);
	}
	...
	public void OnInputEntered(TMP_InputField input)
	{
		...
		if (input == m_widthInput)
		{
			...
			{
				input.SetTextWithoutNotify(m_widthInputText);
			}
			else
			{
		...
		else if (input == m_heightInput)
		{
			if (CheckIfNum(input.text) == false)
			{
				input.SetTextWithoutNotify(m_heightInputText);
			}
			else
			{
	...
	void Update()
	{
		...
		if (m_enabled && m_selectInput.WasReleasedThisDynamicUpdate())
		{
			...
			{
				if (m_changeCheckboxValue == null && !m_displayModeChanging)
